...
bash ./docker_ip.sh node2 brvxlan0 eth1 192.168.0.102/24
bash ./docker_ip.sh node2 brvxlan1 eth2 192.168.100.102/24
sleep 35
docker exec -ti node2 /bin/bash -c 'mkdir /u01'
docker exec -ti node2 /bin/bash -c 'echo "/dev/loop32 /u01    ext4    defaults   0 0" >> /etc/fstab'
docker exec -ti node2 /bin/bash -c 'mount -a'
docker exec -i node2 /bin/bash -c 'cat >/root/create_oraclehome.sh' <./create_oraclehome.sh
docker exec -ti node2 sh /root/create_oraclehome.sh
